...
def encode(s: str, max_length=192):
    if s.startswith("\uEE00"):
        s = s + "</s>"
    else:
        s = "<s>" + s + "</s>"
    e = tokenizer.encode(s, max_length=max_length, truncation=True, padding='max_length', pad_to_max_length=True)
    return e
...
def tokenize(input_file, val_filename, train_filename):
    ...
    max_length = 192
    train_tokens = [encode(line, max_length) for line in train_lines]
    val_tokens = [encode(line, max_length) for line in val_lines]
    ...
    maxtok = (2**15)*3
    assert maxtok / max_length == maxtok // max_length

    write_datafile(val_filename, val_tokens_flat, maxtok)
    print(f"Saved {len(val_tokens_flat)} tokens to {val_filename}")
    write_datafile(train_filename, train_tokens_flat, maxtok)
